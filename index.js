...
function getToken() {
    ...
    throw new Error("Cider API token not configured. Set CIDER_API_TOKEN in .env or cider.key in ~/.pi/agent/auth.json");
}
async function ciderRequest(path, method = "GET", body) {
...
export default function (pi) {
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, _params, _signal) {
            try {
                const response = await fetch(`${CIDER_BASE_URL}/api/v1/playback/active`, { headers: { apitoken: getToken() } });
                if (response.status === 204) {
                ...
                return toolError(`Cannot connect to Cider: ${error instanceof Error ? error.message : "Unknown error"}\n\nMake sure Cider is running and the RPC server is enabled.`);
            }
        },
    });
    pi.registerTool({
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            level: Type.Optional(Type.Number({
                description: "Volume level between 0 and 1 (optional - if omitted, returns current volume)",
            })),
        }),
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params, _signal) {
            ...
            if (data?.data?.results?.songs?.data) {
                results.push("Songs:");
                data.data.results.songs.data.slice(0, 5).forEach((song, i) => {
                    results.push(`  ${i + 1}. ${song.attributes.name} - ${song.attributes.artistName} (${song.id})\n     Album: ${song.attributes.albumName}`);
                });
            }
            if (data?.data?.results?.albums?.data) {
                results.push("\nAlbums:");
                data.data.results.albums.data.slice(0, 3).forEach((album, i) => {
                    results.push(`  ${i + 1}. ${album.attributes.name} - ${album.attributes.artistName}`);
                });
            }
            if (data?.data?.results?.artists?.data) {
                results.push("\nArtists:");
                data.data.results.artists.data.slice(0, 3).forEach((artist, i) => {
                    results.push(`  ${i + 1}. ${artist.attributes.name}`);
                });
            }
            if (results.length === 0) {
